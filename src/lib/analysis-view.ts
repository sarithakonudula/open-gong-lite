...
/** The exact lines the keyword extractor emits, which name a topic and stop. */
...
const CATEGORY_VERB =
  /\b(came up|come(s)? up|was (mentioned|discussed|raised|referenced|named|noted)|were (mentioned|discussed|raised|referenced|named)|got (mentioned|raised|discussed))\b/i;

/** One honest sentence about what the call never got to. */
const ABSENCE_TEXT = /\bwas not stated on this call\b/i;

/**
 * Does this sentence carry anything a reader did not already know from the
 * section header? A number, a name, or a quoted phrase counts. This is the
 * spec's own test, mechanized: cover the citation, and see whether the note
...
/**
 * True when the note names a category instead of stating a finding. These
 * never lead the page: they are what the keyword pass produces when it has
 * nothing to say, and they read as findings while carrying no content.
 */
