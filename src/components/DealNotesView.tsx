...
function SourceRow({
  ...
  return (
    <div className="text-sm">
      ...
      {open && (
        <p className="mt-1 border-l-2 border-signal/30 pl-3 text-fog/85">
...
function NoteItem({
  ...
  return (
    <li className={`space-y-1.5 ${muted ? "border-l-2 border-white/15 pl-3" : ""}`}>
      <p
    ...
    <section className="space-y-3">
      ...
      {section.unverified.length > 0 && (
        <div className="space-y-2 pt-1">
          ...
          <p className="text-sm text-fog/70">{COULD_NOT_VERIFY_EXPLAINER}</p>
      ...
      {section.blocked.length > 0 && (
        <div className="space-y-2 pt-1">
          <p className="text-xs uppercase tracking-[0.16em] text-heat">
      ...
      {section.absenceLine && (
        <p className="text-sm text-mist">{section.absenceLine}</p>
      )}
    </section>
  );
}
      ...
      <div className="space-y-4">
        {view.ownerGroups.map((group) => (
          <div key={group.owner} className="space-y-2">
            <p className="text-sm font-medium text-paper">{group.ownerLabel}</p>
            <ul className="space-y-2">
              {group.steps.map((step) => (
                <li key={step.key} className="text-[0.98rem] text-fog/90">
                  {step.text}
                  {step.due ? (
                    <span className="text-mist"> · due {step.due}</span>
                  ) : null}
      ...
      <div className="space-y-8">
        <header className="space-y-4 animate-rise">
              ...
              <div className="flex flex-wrap gap-2">
                {view.topics.map((topic) => (
                  ...
                  >
                    {topic.label}
                    {topic.timeLabel ? (
                      <span className="text-mist"> {topic.timeLabel}</span>
                    ) : null}
                  </button>
                ))}
              </div>
          ...
          {view.fraction && (
            <p className="text-sm text-fog/90">
              <span className="text-signal">{view.fraction}</span>
        ...
        {notes ? (
          <div className="space-y-10 animate-rise-delay">
            {view.noNotes ? (
              <p className="text-fog/85">{NO_NOTES_LINE}</p>
            ) : (
            ...
            <section className="space-y-3">
              ...
              {view.email.held ? (
                <p className="text-sm text-heat">{EMAIL_HELD_BACK_LINE}</p>
              ) : (
                <>
        ...
        <details className="rounded-2xl border border-white/10 bg-ink-soft/50 px-4 py-3 animate-rise-delay-2">
          <summary className="cursor-pointer text-sm text-mist">
            {RUN_DETAILS_SUMMARY}
          </summary>
          <div className="mt-3 space-y-2 text-sm text-fog/75">
